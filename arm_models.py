from math import sin, cos, tan, acos, atan, asin, sqrt
import numpy as np
...
class TwoDOFRobot:
    ...
    def inverse_jacobian(self, theta: list = None):
        """
    ...
    def calc_inverse_kinematics(self, EE: EndEffector, soln=0):
        ...
        if soln is 0:
            t2 = acos((L**2 - l1**2 - l2**2) / (2 * l1 * l2))
            t1 = atan(y / x) - acos((l2**2 - L**2 - l1**2) / (-2 * L * l1))
        else:
            t2 = -acos((L**2 - l1**2 - l2**2) / (2 * l1 * l2))
            t1 = atan(y / x) + acos((l2**2 - L**2 - l1**2) / (-2 * L * l1))
    ...
    def calc_numerical_ik(self, EE: EndEffector, tol=0.01, ilimit=50):
        ...
        for i in range(ilimit):
            error = np.subtract([x, y], self.solve_forward_kinematics(self.theta))
            if sqrt(error[0]**2 + error[1]**2) > tol:
...
class ScaraRobot:
    ...
    def calc_inverse_kinematics(self, EE: EndEffector, soln=0):
        ...
        if soln is 0:
            t2 = acos((L**2 - l2**2 - l4**2) / (2 * l2 * l4))
            t1 = atan(y / x) - acos((l4**2 - L**2 - l2**2) / (-2 * L * l2))
        else:
            t2 = -acos((L**2 - l2**2 - l4**2) / (2 * l2 * l4))
            t1 = atan(y / x) + acos((l4**2 - L**2 - l2**2) / (-2 * L * l2))
